...
public abstract class JobStoreSupport implements JobStore, Constants {
    ...
    public void schedulerStarted() throws SchedulerException {
        ...
        misfireHandler.initialize();//获取ThreadExecutor 线程管理
     ...
     */
    //将恢复任何失败或misfire的作业，并根据需要清理数据存储。
    ...
    protected void recoverJobs(Connection conn) throws JobPersistenceException {
        try {
            ...
            //更新不一致的作业状态     先修改状态，将 ACQUIRED 和 BLOCKED ---> WAITING
   ...
   // 在数据库中查找一定时间范围内将会被触发的trigger.
    // 参数的意义如下:参数1:nolaterthan = now+3000ms,即未来30s内将会被触发.参数2 最大获取数量,
    // 大小取线程池线程剩余量与定义值得较小者.参数3 时间窗口 默认为0,程序会在nolaterthan后加上窗口大小来选择trigger.
    // quratz会在每次触发trigger后计算出trigger下次要执行的时间,并在数据库QRTZ2_TRIGGERS中的NEXT_FIRE_TIME字段中记录.
    // 查找时将当前毫秒数与该字段比较,就能找出下一段时间内将会触发的触发器.查找时,调用在JobStoreSupport类中的方法:
    public List<OperableTrigger> acquireNextTriggers(final long noLaterThan, final int maxCount, final long timeWindow)
        ...
        //该方法关键的一点在于执行了executeInNonManagedTXLock()方法,这一方法指定了一个锁名,两个回调函数.
        // 在开始执行时获得锁,在方法执行完毕后随着事务的提交锁被释放.在该方法的底层,使用 for update语句,
        // 在数据库中加入行级锁,保证了在该方法执行过程中,其他的调度器对trigger进行获取时将会等待该调度器释放该锁.
        // 此方法是前面介绍的quartz集群策略的的具体实现,这一模板方法在后面的trigger触发过程还会被使用.
        return executeInNonManagedTXLock(lockName, //executeInNonManagedTXLock为关键方法
                new TransactionCallback<List<OperableTrigger>>() {
                    public List<OperableTrigger> execute(Connection conn) throws JobPersistenceException {
                        // 到了这里，继续往下,这里返回需要下次执行的Trigger  重点分析以下方法！！！！！！
        ...
        do {
            ...
            try {
                ...
                for(TriggerKey triggerKey: keys) {
                    ...
                    OperableTrigger nextTrigger = retrieveTrigger(conn, triggerKey);//查询Trigger 并封装！retrieveTrigger 的 sql
                    if(nextTrigger == null) {
                    ...
                    if (job.isConcurrentExectionDisallowed()) {//相关联的Job类是否携带DisallowConcurrentExecution注释。
