...
* [群集容错参考](http://dubbo.apache.org/zh-cn/docs/user/demos/fault-tolerent-strategy.html)；当我们一个服务调用远程服务，如果这个服务失败了怎么办？在集群调用失败时，Dubbo 提供了多种容错方案，缺省为 failover 重试。多种容错机制可以参考文档；
...
````
  1, 容错一般与Hystrix配合使用
     ...
     整合步骤：
      1，导入Hystrix
      2，开启服务容错
      3，在可能出错的方法标记HystrixCommand注解
      
      注意：在服务方与消费都需要使用Hystrix
  
````

* RPC与Netty
* ![RPC原理](img/009.png)  ![](img/010.png)

````
  RPC:远程服务调用
      发起远程请求-》客户端代理对象-》客户端代理对象将请求发给服务器端 -》服务端代理就请求转给服务端的服务-》调用完后，服务端代理就结果返回到客户端代理
      
  RPC框架的作用就是把中间的过程封装起来；    
...
````
  ...
  Dubbo底层通过采用Netty的NIO(非阻塞IO)
  阻塞式与非阻塞式的区别：
     1，阻塞式：请求到来就开一个新线程；
     2，非阻塞式：通道 buffer 选择器（多路复用）；每个选择器注册了很多通道，每一个请求进来，都可以弄成通道，选择器对通道进行监听各种事件；比如监听到开始连接、连接成功；
      一个选择器可以监听多个通道，等我们感兴趣的事件发生以后，我们再开线程去做这个事情。（多路复用）
...
Connect（连接就绪）、Accept（接受就绪）、Read（读就绪）、Write（写就绪）
  ...
  Netty就是基于NIO原理实现的
````
* ![netty的基本原理](img/011.png)

````
  基本原理或步骤：
  1，netty服务启动，并绑定端口
  2，初始化服务器的通道
  3，通道初始化完成后，注册到selector
  4,注册完成以后，监听通道accept事件
  5，与客户端连接通道生成NioScoketChannel
  6，。。。
  7，当读、写都准备就绪以后，就开始处理事件（抛给对应的任务队列）
  
````

##### Dubbo的原理(断点跟踪)
* Dubbo的原理-[框架设计](http://dubbo.apache.org/zh-cn/docs/dev/design.html)
* Dubbo的原理-启动解析、加载配置信息

````
   spring解析配置文件的入口：DubboBeanDefinitionParser
   每个配置都会到这里解析
   每一个标签都可能对应一个类，所有标签类都在DubboNamespaceHandler中注册
